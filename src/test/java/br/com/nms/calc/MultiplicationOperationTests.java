...
class MultiplicationOperationTests {
	...
	void multiplicationWithTwoInteger() throws Exception {
		...
		final ResultActions resultActions = mockMvc.perform(MockMvcRequestBuilders.post("/calc/multiply")
				.contentType(MediaType.APPLICATION_JSON_VALUE)
				.content(new Gson().toJson(operation)));

		resultActions.andExpect(status().isOk());
		resultActions.andExpect(jsonPath("$.result", is(10)));
	}
	...
	void multiplicationWithNegativeInteger() throws Exception {
		...
		resultActions.andExpect(status().isOk());
		resultActions.andExpect(jsonPath("$.result", is(-8)));
	}
	...
	void multiplicationWithTwoNegativeInteger() throws Exception {
		...
		resultActions.andExpect(status().isOk());
		resultActions.andExpect(jsonPath("$.result", is(20)));
	}
